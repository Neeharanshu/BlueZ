...
//Added by Neehar
static const char *const mac_ids[]={
	"88:6B:0F:30:5D:E2",
	"88:6B:0F:30:55:CC"
};

static const char *const char_id[]={
	"/org/bluez/hci0/dev_88_6B_0F_30_5D_E2/service000d/char000e",
	"/org/bluez/hci0/dev_88_6B_0F_30_55_CC/service0037/char0038"

};
const char *fname="/home/pi/data.txt";
//Added by Neehar
						...
						DBusMessageIter *iter)
{
	...
	case DBUS_TYPE_OBJECT_PATH:
		...
		rl_printf("%s%s: %s\n", label, name, valstr);
		rl_printf("I am printing a path now");
		break;
	case DBUS_TYPE_BOOLEAN:
		dbus_message_iter_get_basic(iter, &valbool);
		rl_printf("%s%s: %s\n", label, name,
					valbool == TRUE ? "yes" : "no");
	//	rl_printf("I am printing booleans now");
		break;
	case DBUS_TYPE_UINT32:
		dbus_message_iter_get_basic(iter, &valu32);
		rl_printf("%s%s: 0x%06x\n", label, name, valu32);
	//	rl_printf("I am printing uint32s");
		break;
	...
	//Added by Neehar
		static int bytenum;
		static int byte_store;
		FILE *f = fopen(fname,"a");
		i=i+1;
		if(i==1)
		{	unsigned char byte_read;
			dbus_message_iter_get_basic(iter, &byte_read);
			bytenum=byte_read;
			byte_store=bytenum;
			fprintf(f, "%s ", dt);
		}
		...
		{
			if(((i%2)==0)&&((i%13)!=0)&&((i%14)!=0)&&((i%15)!=0)&&((i%16)!=0)&&((i%17)!=0)&&((i%18)!=0))
			{
				signed char byte_read_1;
				dbus_message_iter_get_basic(iter, &byte_read_1);
				if(bytenum != 0)
				{
					bytenum=((byte_read_1)*256);
				}
			//	byte_store=bytenum;
				byte_store=(byte_store+bytenum);
	//			rl_printf("%i %i %s", bytenum, byte_store, byte_read);
				fprintf(f,",%i ", byte_store);
			}
			else
			{
				unsigned char byte_read_2;
                dbus_message_iter_get_basic(iter, &byte_read_2);
				bytenum=byte_read_2;
				byte_store=bytenum;
			//	fprintf(f, "%i,,",bytenum);
				if ((i%18)==0)
                                {
                        //      byte_store=(byte_store+(((bytenum/10)*10)+(bytenum%10)
					fprintf(f, "\n");
                                        byte_store=0;
					i=0;
                        	}
		...
		fclose(f);
	//	rl_printf("Printing Bytes\n");
//Added by Neehar
		break;
	case DBUS_TYPE_VARIANT:
		dbus_message_iter_recurse(iter, &subiter);
		print_iter(label, name, &subiter);
	//	rl_printf("Printing a variant");
		break;
...
static void print_property(GDBusProxy *proxy, const char *name)
{
	...
	if (g_dbus_proxy_get_property(proxy, name, &iter) == FALSE)
		return;
	rl_printf("Called from print_property\n");
...
static void cmd_list_attributes(const char *arg)
{
	...
	proxy = find_device(arg);
	if (!proxy){
		rl_printf("Reached here");
		return;}
rl_printf("Reached here with device %s\n", arg);
...
//Added by neehar
static void bleconnect(const char *num)
{
	int id=atoi(num);
	GDBusProxy *proxy;
	rl_printf("id is %i and num is %s", id, num);
	cmd_connect(mac_ids[id]);
	rl_printf("Did I come back here?");
	proxy=gatt_select_attribute(NULL, char_id[id]);
}

static void cmd_aros_notify(const char *arg, bool enable){
	GDBusProxy *proxy;
	int id=atoi(arg);
        proxy=gatt_select_attribute(NULL, char_id[id]);
        if(proxy){
	...
	gatt_notify_attribute(proxy, enable ? true:false);

}

static void cmd_aros_notify_start(const char *arg){
	cmd_aros_notify(arg,true);
...
static const struct {
	...
	{ "connect",      "<dev>",    cmd_connect, "Connect device",
							dev_generator },
//Added by neehar
	{ "bleconnect", "<device_id>", bleconnect, 
				"Connect to ARoS Whitelist Device ID"},
	{"notifi", "dbus_path", cmd_aros_notify_start, "Start Notifying from attribute"},
//Added by neehar
...
static void rl_handler(char *input)
{
	...
	rl_printf("Command recieved %s", cmd);
	if (!cmd)
		goto done;
...
int main(int argc, char *argv[])
{
	...
	rl_printf("starting new loop\n");
	dbus_conn = g_dbus_setup_bus(DBUS_BUS_SYSTEM, NULL, NULL);
	rl_printf("setting up new connection\n");
	g_dbus_attach_object_manager(dbus_conn);
	rl_printf("Attaching new object\n");
	setlinebuf(stdout);
	rl_printf("Set line buffer executed\n");
	rl_attempted_completion_function = cmd_completion;
	rl_printf("Ran cmd_completion\n");
	rl_erase_empty_line = 1;
	rl_printf("Erase Empty Line\n");
	...
	rl_printf("Setup signal executed\n");
	client = g_dbus_client_new(dbus_conn, "org.bluez", "/org/bluez");
	rl_printf("created new client");
	g_dbus_client_set_connect_watch(client, connect_handler, NULL);
	rl_printf("Set connect watch\n");
	g_dbus_client_set_disconnect_watch(client, disconnect_handler, NULL);
	rl_printf("Set disconnect watch\n");

	g_dbus_client_set_signal_watch(client, message_handler, NULL);
	rl_printf("Set signal watch\n");

	g_dbus_client_set_proxy_handlers(client, proxy_added, proxy_removed,
							property_changed, NULL);
	rl_printf("Set proxy handlers\n");

	g_dbus_client_set_ready_watch(client, client_ready, NULL);
	rl_printf("Set ready watch\n");

	g_main_loop_run(main_loop);
	rl_printf("run main loop\n");

	g_dbus_client_unref(client);
	rl_printf("Set unref client\n");
	g_source_remove(signal);
	rl_printf("remove signal\n");

	if (input > 0)
		g_source_remove(input);
		rl_printf("input is greater than 0\n");

	rl_message("");

	rl_callback_handler_remove();
	rl_printf("remove handler\n");

	dbus_connection_unref(dbus_conn);
	rl_printf("unref dbus connection\n");
	g_main_loop_unref(main_loop);
	rl_printf("unref main loop\n");
